...
from .dtx_msg import DTXMessage, MessageAux, dtx_message_header, object_to_aux
...
class DTXServer(BaseRpc):
    ...
    def _start(self):
        ...
        if self._running:
            return True
        self._running = True
        self._recv_thread = Thread(target=self._receiver, name="DTXServer")
    ...
    def call(self, channel: str, selector: str, *auxiliaries) -> DTXMessage:
        channel_id = self.make_channel(channel)
        return self._call(True, channel_id, selector, *auxiliaries)
    ...
    def _call(self, sync: bool, channel_id: int, selector: str, *auxiliaries) -> DTXMessage:
        """
        :param sync: 是否回调
