...
from ..util.exceptions import PairingError, NotTrustedError, FatalPairingError, CannotStopSessionError
from ..util.exceptions import StartServiceError, InitializationError
from ..util.log import default as log
from ..util.plist_service import PlistService
from ..util.ca import make_certs_and_key
from ..util.usbmux import MuxDevice, UsbmuxdClient
from ..util import DictAttrProperty

from .image_mounter import MobileImageMounter
...
class LockdownClient:
    ...
    def __init__(
        ...
        self.svc = PlistService(udid=udid, device=device, network=network)
    ...
    def identifier(self):
        ...
        if self.udid:
            self._identifier = self.udid
            return self._identifier
        elif self.unique_chip_id:
            self._identifier = f'{self.unique_chip_id:x}'
            return self._identifier
    ...
    def _request_developer_image_dir(self):
        ...
        mac_developer_dir = f"/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/" \
                            f"DeviceSupport/{version}"
...
def read_home_file(folder_name: str, filename: str) -> Optional[bytes]:
...
def write_home_file(folder_name: str, filename: str, data: bytes) -> str:
    path = get_home_path(folder_name, filename)
